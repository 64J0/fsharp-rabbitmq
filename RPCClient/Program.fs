...
type RpcClient() =
    ...
    member __.CallAsync(message: string, ?cancellationToken: CancellationToken) =
        ...
        let props =
            channel.CreateBasicProperties(CorrelationId = correlationId, ReplyTo = replyQueueName)
        ...
        canToken.Register(fun _ -> callbackMapper.TryRemove correlationId |> ignore)
        |> ignore
...
let main (args: string[]) : int =
    printfn "RPC Client"
    let n = if Array.isEmpty args then "30" else Array.head args
